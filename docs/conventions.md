...
- `LICENSE` — MIT verbatim. SPDX header (`SPDX-License-Identifier: MIT`) at the top of every source file (mechanical sweep deferred to a follow-up PR; the header is not yet on every file).
...
**MIT.** Apache-2.0 was the original pick (patent grant for audio/wake-word/prompt-engineering surface, cleaner SBOMs for enterprise downstream). Re-examined for tend specifically: the patent surface is mostly in *upstream* libraries (openWakeWord, pipecat, pyaec) — tend itself is glue. MIT is dominant in AI-OSS (Ollama, llama.cpp, LangChain), shorter to read, and lower-friction for casual contributors. Apache-2.0's protections aren't load-bearing for a single-author personal-infra project.

If tend ever attracts corporate contributors with patents in this space, a relicense to Apache-2.0 is straightforward while there's still a single copyright holder — defer the decision until it's a real concern.

## Distribution

- **Primary:** PyPI. Distribution name is **`tend-assistant`** (the bare name `tend` was already registered); import name remains `tend`. Recommended user install: `pipx install tend-assistant` (isolated venv, on `$PATH`). Document this. macOS users need `brew install portaudio` first; WebRTC AEC3 is opt-in: `pipx inject tend-assistant webrtc-audio-processing` (installs the `[aec-webrtc]` extra).
