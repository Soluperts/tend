...
- **Mac AEC via Apple VoiceProcessingIO (custom audio transport).** Required follow-up to the macOS port. The shipped speex-based AEC (`pyaec`) gives 12–26 dB suppression on built-in MacBook mic+speaker — enough that VAD ignores the residual with tuning, not enough that Deepgram's cloud STT stops transcribing fragments of the bot's own voice into the LLM context. The clean fix is a `pipecat.BaseTransport` subclass that uses `AVAudioEngine` with `setVoiceProcessingEnabled(True)` instead of PyAudio/PortAudio. The engine handles AEC + NS + AGC internally with OS-grade quality — same path FaceTime and Voice Memos use. Probably upstreamable to pipecat as `pipecat.transports.local.apple_voice` (parallels their existing `LocalAudioTransport`). Real cost: ~1–2 days of PyObjC + CoreAudio plumbing, mostly de-risked by writing a standalone proof-of-concept before the pipecat integration. Until this lands, Mac users on built-in mic+speaker should be aware AEC is limited; external USB speakerphones with hardware AEC sidestep the issue.
