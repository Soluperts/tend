...
- **Webhook port: friendlier default + EADDRINUSE error.** The default `[webhook] port = 7331` collides with VS Code's helper port — `python -m tend` crashes with a raw aiohttp traceback if VS Code is running. Two small fixes: (a) change the default to something in the IANA dynamic range (49152–65535) that's less likely to clash; (b) wrap `webhook.start()` so EADDRINUSE prints a clear "another process is holding port N — set `[webhook] port` in tend.toml or stop the conflicting process" message instead of dumping the traceback. *~30 minutes.*
